...
A **dermatologist market-intelligence dashboard** for Guntur, Andhra Pradesh. It turns 80 high-intent
Google searches into a competitive map of local skin clinics, scores each on a 0–100 **Opportunity
Index** (local demand weighed against online-presence gaps), and surfaces the 10 with the most untapped
potential. Built to be sold to clinics as a "where you stand vs. the market" report.
...
## Documentation
- **[CLAUDE.md](CLAUDE.md)** — start here: constraints, run commands, module map, environment gotchas, current state.
- **[ARCHITECTURE.md](ARCHITECTURE.md)** — components, data flow, layers, resilience patterns.
- **[DESIGN.md](DESIGN.md)** — the "Quiet Precision" visual system + scoring/UX rationale.
- **[SESSION_LOG.md](SESSION_LOG.md)** — what was built, phase by phase · **[PROMPT.md](PROMPT.md)** — the next task.
...
- **Tab 1 — Queries:** paste-in workflow + a sortable/filterable table of all 80 queries (category,
...
```
CLAUDE.md ARCHITECTURE.md DESIGN.md SESSION_LOG.md PROMPT.md   # project docs (start with CLAUDE.md)
app.py                  # Streamlit console (sidebar + pipeline + 4 tabs)
derma_web.py            # build + open the premium web dashboard
run_pipeline.py         # headless Maps scrape (resume-safe)
collect_extras.py       # --reviews (NLP) and --web (headful Google web)
config.py               # constants, paths, scraper settings (no keys)
modules/
  query_generator.py    # prompt builder + robust paste parser + xlsx
  maps_collector.py     # Playwright Maps scraper + mock mode + OSM fallback + detail cache
  vulnerability.py      # 0–100 opportunity score + 60/40 web blend + notes + top-10 export
  analytics.py          # KPIs + chart builders + data-prep helpers
  web_collector.py      # Google web SERP (headful, human-in-the-loop) + clinic matching
  reviews_collector.py  # Google Maps reviews scraper (resume-safe)
  reviews_nlp.py        # free VADER NLP: sentiment, themes, pain points, referral rate
  storage.py            # JSON state store (reload without re-scraping)
components/             # Streamlit tabs (queries, results, analytics, vulnerable) + _format
web/                    # premium UI: build_web.py, template.html, styles.css, app.js, vendor/, dist/
data/                   # .xlsx exports + Full Page Screenshots/ (Google SERP captures)
.cache/                 # scrape caches + JSON app state
docs/                   # redesign audit + build plan
tests/                  # pytest suite (run: python -m pytest)  — 97 tests
```
