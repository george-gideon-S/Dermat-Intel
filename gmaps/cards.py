...
# Imported, not redefined. Keeping a private copy here meant two regexes for the same field
# drifted apart: this module accepted "/g/" only while fields.py accepted any "/x/" namespace,
# and whichever ran last overwrote the other - silently truncating kg_mid on 18 of 98 real cards.
from gmaps.fields import FID_RE as _FID_RE
from gmaps.fields import LL_RE as _LL_RE
from gmaps.fields import MID_RE as _MID_RE
from gmaps.fields import PID_RE as _PID_RE
...
_BOOKING_VENDORS = {
    "justdial.com": "justdial", "remedoapp.com": "remedo", "healthplix.com": "healthplix",
    "book.healthplix.com": "healthplix", "practo.com": "practo", "apollo247.com": "apollo247",
    "bajajfinservhealth.in": "bajaj", "docon.co.in": "docon", "zocdoc.com": "zocdoc",
    "calendly.com": "calendly", "nexhealth.com": "nexhealth",
}


def _parse_booking(card) -> dict:
    """The 'Book online' action on a results card.

    Lives on the RESULTS list, not on any tab of the place page, so it is only ever available
    here - open the place and it is gone. Measured 6/98 cards in the Guntur capture.

    Commercially it is a maturity signal: a clinic wired into an online booking system is further
    along digitally than one reachable only by phone. The vendor matters too - a JustDial booking
    link means the clinic is renting someone else's funnel, the same borrowed-vs-owned distinction
    that separates an aggregator profile from a real website.
    """
    a = card.select_one("a.A1zNzb")
    if a is None:
        return {"has_online_booking": False, "booking_url": "", "booking_vendor": ""}
    href = (a.get("href") or "").strip()
    m = re.match(r"https?://([^/?#]+)", href, re.I)
    host = (m.group(1).lower() if m else "").lstrip("www.")
    vendor = ""
    for dom, name in _BOOKING_VENDORS.items():
        if host == dom or host.endswith("." + dom):
            vendor = name
            break
    return {"has_online_booking": True, "booking_url": href,
            # unknown vendors keep the host so a new booking platform shows up in the data
            # instead of silently disappearing into an empty string
            "booking_vendor": vendor or host}


def _parse_service_options(card) -> dict:
    """Service-option chips Google prints on the card (e.g. 'On-site services').

    Measured 10/98 cards. Kept as raw labels: the vocabulary varies by trade and country, so
    normalising it here would throw away detail we cannot yet interpret.
    """
    # Scanned over the whole card, not a specific row: Google places these chips in a different
    # container than the category/status rows, and an over-precise selector found none of the
    # ten that are demonstrably present in the capture.
    text = _txt(card)
    opts = [phrase for phrase in
            ("On-site services", "Online appointments", "Onsite services",
             "Language assistance", "Identifies as women-owned", "Identifies as Black-owned",
             "Wheelchair accessible entrance", "Wheelchair accessible parking lot")
            if phrase.lower() in text.lower()]
    return {"service_options": opts or None}
...
def parse_card(card) -> dict:
    ...
    # A card that explicitly says "No reviews" is a MEASURED zero, not an unknown. Leaving it as
    # None merged it with the ~20 cards whose count simply was not rendered in Google's first
    # batch, so a genuine zero and a failed read became the same value.
    if no_reviews and reviews is None:
        reviews = 0
