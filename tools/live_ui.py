...
.card.relevant{border-left-color:var(--ok)}
.card.adjacent{border-left-color:var(--warn)}
...
.tag.relevant{color:var(--ok);border-color:#1c4429}
.tag.adjacent{color:var(--warn);border-color:#4a3a12}
.tag.irrelevant{color:var(--bad);border-color:#5a2222}
...
async function loadReviews(key, el){
  try{
    ...
    el.innerHTML = rv.length ? rv.map(v=>`<div class="rev"><div class="rh">
      ...
      ${v.maybe_truncated?'<span class="tag mi">may be cut off</span>':''}
...
function renderStats(){
  ...
  const elapsed = STATUS.started_at ? (Date.now()/1000 - STATUS.started_at) : 0;
  $('#stats').innerHTML =
    el(hhmmss(elapsed),'running for','time') +
